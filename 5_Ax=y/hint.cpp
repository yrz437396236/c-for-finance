...
void trim_the_solution_set(Matrix A, ColumnVector y)
{
	...
	
	{
		...
		{			
			int k = 0;
			vector <vector <double> > B;
			for (int i = 0; i < rank_of_solution_set; i++)
			{
				vector <double> temp;
				B.push_back(temp);
			}
			...
			for (int i = 0; i < solution.ncols(); i++)
			{
				if (did_i_pick_this[i] == true)
				{
					...
					{
						B[k].push_back(solution(j+1, i+1));
					}
					k++;
				}
			}
...
void process_data()
{
	int no_of_rows, no_of_cols;
	double element;
	cout << "didn't find input, automatally use input1" << endl;
	// First command line input is the name of the input file
	...
	if (input_file.is_open()) // If the input file exists in the local directory
	{
		...
		//size_t size;
		//path = getcwd(path, size);
		cout << "Input file: " << "input1" << " does not exist in present folder" << endl;
	}
}
